...
//		System.out.println(tokens);
		...
		for (String str : tokens) {
			if (isWord(str)) {
				int count = 0;
				if (str.length() == 1) {
					if (Arrays.asList(array).contains(str.charAt(0))) {
						count += 1;
					}
				}
				for (int i = 0; i < str.length()-1; i++) {
					if (Arrays.asList(array).contains(str.charAt(i))) {
						if (!Arrays.asList(array).contains(str.charAt(i+1)) || i+1 == str.length()-1) {
							count++;
						}
					} else if (Arrays.asList(array).contains(str.charAt(i+1)) && i+1 == str.length()-1) {
