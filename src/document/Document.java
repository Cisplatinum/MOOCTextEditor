...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int count = 0;
		Character[] array = {'a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U'};
		if (word.length() == 1) {
			if (Arrays.asList(array).contains(word.charAt(0))) {
				count += 1;
			}
		}
		for (int i = 0; i < word.length()-1; i++) {
			if (Arrays.asList(array).contains(word.charAt(i))) {
				if (Arrays.asList(array).contains(word.charAt(i+1)) && i+1 != word.length()-1) {
					continue;
				} else {
					count++;
				}
			} else if (Arrays.asList(array).contains(word.charAt(i+1)) && i+1 == word.length()-1) {
